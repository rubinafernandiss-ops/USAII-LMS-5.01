...
async function main() {
  ...
  app.get('/api/health', (_req, res) => res.json({ app: APP_ID, status: 'ok', version: '5.0.0', time: new Date().toISOString() }));
  app.use('/api/auth', authRouter);
  app.use('/api/learner', learnerRouter);
  app.use('/api/public', publicRouter);
  app.use('/api/public', catalogRouter);
  ...
  console.log('');
  console.log('  ==============================================');
  console.log('   USAII(R) Intuitive LMS 5.0 is running');
  ...
  console.log('     (The instructor account now runs the institute too — there is no separate administrator.)');
  console.log('');
