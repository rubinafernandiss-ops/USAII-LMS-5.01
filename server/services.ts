...
/** Notify the instructor who owns this course; if they are gone, every active instructor. */
export function notifyCourseStaff(course: Course, n: Omit<Notification, 'id' | 'userId' | 'createdAt' | 'read'>) {
  const targets = db().users.filter((u) => u.active && u.id === course.createdBy && isStaff(u.role));
...
/** The instructor hears about learners who start to fall behind, at most once a week each. */
function generateStaffAlerts(staff: UserRecord) {
  const d = db();
  const now = Date.now();
  const week = Math.floor(now / (7 * 86_400_000));
  // The instructor now runs the institute, so every published course is theirs to watch.
  const courses = d.courses.filter((c) => c.status === 'published');
  let created = 0;
...
export function validateCourse(input: any, existing: Course | undefined, actor: UserRecord): Course {
  ...
  const now = nowIso();
  // One instructor now runs the institute, so they set the course fee themselves.
  const price = Math.max(0, Math.min(4999, Number(input.price) || 0));
  return {
