    ...
    <Card className="mx-auto max-w-2xl !p-7">
      <div className="space-y-5">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Field label="Course fee (USD)" hint="(0 = free)">
            <div className="flex items-center gap-2">
              <span className="text-ink-soft">$</span>
              <Input type="number" min={0} max={4999} value={c.price} onChange={(e) => set({ price: Math.max(0, Math.min(4999, Number(e.target.value) || 0)) })} className="!w-28" />
            </div>
          </Field>
          <Field label="Duration" hint="shown in the catalog">
            <Input value={c.durationLabel} onChange={(e) => set({ durationLabel: e.target.value })} placeholder="e.g. 10 days · 1 hour a day" />
          </Field>
          <Field label="Level">
            <Input value={c.level} onChange={(e) => set({ level: e.target.value })} placeholder="e.g. Everyone, Intermediate" />
          </Field>
          <Field label="Color">
            <div className="flex gap-2">
