...
import { Award, BookOpen, CheckCircle2, ChevronDown, Circle, Clock, Download, FileCheck2, FileText, Film, Headphones, Link2, Lock, PlayCircle } from 'lucide-react';
import { useState } from 'react';
import { flattenLessons, progressOf } from '../../shared/analytics';
import { downloadStudyGuidePdf } from '../lib/pdf';
import type { Course } from '../../shared/types';
import { fmtMinutes, scoreColor } from '../lib/format';
      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-5 grid gap-5 border-t border-line pt-5 md:grid-cols-2">
          ...
          <div>
            ...
            <Button variant="secondary" size="sm" className="mt-3" icon={<Download className="h-4 w-4" />} onClick={() => downloadStudyGuidePdf(c)}>
  ...
  return (
    <Card>
      ...
      <div className="mt-4 space-y-2">
        {course.studyPlan?.url && (
          <a
            href={course.studyPlan.url}
            target="_blank"
          ...
          >
            ...
            <span className="flex-1">
              <span className="block font-semibold">Study Plan (PDF)</span>
              <span className="block text-xs text-ink-faint">From your instructor: how this course is paced, week by week.</span>
            </span>
          </a>
        )}
        <button
          onClick={() => downloadStudyGuidePdf(course)}
        ...
        >
          ...
          <span className="flex-1">
            <span className="block font-semibold">Study Guide (PDF)</span>
            <span className="block text-xs text-ink-faint">The whole course as a PDF you can keep, print or read offline.</span>
          </span>
        </button>
        {items.map((r) => {
...
export default function Learning() {
  ...
  const mastery = new Map(snapshot.mastery.map((m) => [m.lessonId, m]));
                    ...
                    <li key={l.id}>
                      ...
                      >
                        ...
                        <span className="hidden shrink-0 text-right sm:block">
                          {t && t.bestCheck !== null ? (
                            <span className="block font-display font-bold" style={{ color: scoreColor(t.score) }}>
                              {t.score}%
                            </span>
                          ) : (
                            <span className="block text-sm text-ink-faint">{l.estimatedMinutes} min</span>
                          )}
                          <span className="block text-[11px] text-ink-faint">{t?.state ?? ''}</span>
                        </span>
                      </button>
                    </li>
                  );
