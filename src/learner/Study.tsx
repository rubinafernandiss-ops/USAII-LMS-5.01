...
import type { CheckQuestion, Enrollment, LearnerSnapshot, Lesson, LessonProgress } from '../../shared/types';
          ...
          {sub.feedback ? (
            <div className="mt-5 rounded-2xl border border-npurple/20 bg-npurple-soft/50 p-4">
              <div className="text-sm font-semibold text-npurple">
                Feedback from {sub.feedbackBy} · {timeAgo(sub.feedbackAt)}
              </div>
              <p className="mt-1 whitespace-pre-line text-[15px]">{sub.feedback}</p>
            </div>
          ) : (
            <p className="mt-4 text-xs text-ink-faint">Your instructor can leave feedback here. You will get a notification.</p>
          )}
        </Card>
      ) : readOnly ? (
...
export default function Study({ courseId, lessonId, initialStep, autoListen }: { courseId: string; lessonId: string; initialStep?: string; autoListen?: boolean }) {
  ...
  const mastery = item.snapshot.mastery.find((m) => m.lessonId === lesson.id);
    ...
    <div className={cx('grid gap-8', prefs.focus ? 'grid-cols-1' : 'xl:grid-cols-[minmax(0,1fr)_300px]')}>
      <div className="min-w-0">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {mastery && mastery.bestCheck !== null && (
            <Pill color={mastery.score >= 80 ? 'green' : mastery.score >= 60 ? 'blue' : 'purple'}>
              {mastery.state} · {mastery.score}%
            </Pill>
          )}
          {p.status === 'done' && (
            <Pill color="green">
