...
interface AcquiredSkill {
  lesson: Lesson;
  /** A mastered skill was checked at 85% or better. */
  mastered: boolean;
  score: number | null;
}
...
export default function SkillTree({ item }: { item: CourseItem }) {
  ...
  const { skills, total } = useMemo(() => {
    ...
    flat.forEach((f) => {
      ...
      const m = mastery.get(f.lesson.id);
      const score = m && m.bestCheck !== null ? m.score : null;
      acquired.push({ lesson: f.lesson, mastered: score !== null && score >= 85, score });
    });
    return { skills: acquired, total: flat.length };
  }, [course, enrollment, snapshot.mastery]);
        ...
        <div className="relative overflow-hidden border-b border-line bg-gradient-to-r from-nblue-soft via-npurple-soft to-npink-soft/70 px-5 py-5 sm:px-6">
          ...
          <div className="relative flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="text-right">
              ...
              <div className="text-[12px] font-semibold text-ink-soft">{masteredCount ? `${masteredCount} mastered` : 'skills acquired'}</div>
            </div>
          </div>
            ...
            <div className="flex flex-wrap items-start gap-2 sm:gap-3">
              {skills.map((s, i) => (
                <motion.button
                  ...
                  title={`${s.lesson.title} — ${s.mastered ? 'Mastered' : 'Acquired'}`}
                ...
                >
                  <span
                    className={cx(
                      ...
                      s.mastered ? 'bg-gradient-to-br from-ngreen to-nblue shadow-ngreen/30 ring-ngreen/25' : 'bg-gradient-to-br from-nblue to-npurple shadow-npurple/30 ring-npurple/20',
                      s.lesson.id === openId && 'scale-105',
                    )}
                  >
                    {s.mastered ? <Star className="h-7 w-7" /> : <CheckCircle2 className="h-7 w-7" />}
                    {s.score !== null && (
                      <span className="absolute -bottom-2 rounded-full bg-white px-1.5 text-[10px] font-bold shadow" style={{ color: s.mastered ? '#00804F' : '#1F6BFF' }}>
                        {s.score}%
                      </span>
                    )}
                  </span>
          ...
          <motion.div key={open.lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
            <Card className="border-npurple/30">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className={cx('rounded-full px-3 py-1 text-[12px] font-bold', open.mastered ? 'bg-ngreen-soft text-ngreen-ink' : 'bg-nblue-soft text-nblue')}>{open.mastered ? 'Mastered' : 'Acquired'}</span>
              </div>
