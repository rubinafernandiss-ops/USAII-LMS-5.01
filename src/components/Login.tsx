...
import { ArrowRight, CheckCircle2, Compass, Eye, EyeOff, GraduationCap, Lock, Mail, Presentation } from 'lucide-react';
import { useState, type FormEvent } from 'react';
import type { PublicUser } from '../../shared/types';
import { api, tokenStore } from '../lib/api';
import { PublicFooter, PublicHeader, Particles } from '../public/PublicShell';
import { cx, Input, Label, navigate } from './ui';
...
export default function Login({ onAuthed }: { onAuthed: (u: PublicUser) => void }) {
  ...
  // Two doors into the same portal: it tells people they are in the right place.
  const [role, setRole] = useState<'learner' | 'instructor'>('learner');
  const ROLES = [
    { id: 'learner' as const, label: 'Learner', icon: GraduationCap, color: '#1F6BFF', hint: 'Take your course and earn your USAII® micro-credential.', demo: { email: 'alex.rivera@enterprise.com', password: 'Learner@2026' } },
    { id: 'instructor' as const, label: 'Instructor', icon: Presentation, color: '#8B3DFF', hint: 'Teach, guide your learners, and run the institute.', demo: { email: 'instructor@usaii.org', password: 'Instructor@2026' } },
  ];
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-8">
        ...
        <main className="grid flex-1 items-center gap-12 py-8 lg:grid-cols-[1.15fr_1fr]">
          ...
          <div className="text-center lg:text-left">
            ...
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.45, duration: 0.5 }} className="mt-7 flex flex-wrap justify-center gap-3 lg:justify-start">
              <button
                onClick={() => navigate('explore-courses')}
                className="btn-shine flex items-center gap-2 rounded-full bg-gradient-to-r from-nblue via-npurple to-npink bg-[length:200%_100%] bg-left px-6 py-3 font-bold text-white shadow-lg shadow-npurple/30 transition-[background-position] duration-500 hover:bg-right"
              >
                <Compass className="h-4 w-4" /> Explore Courses
              </button>
              <button onClick={() => navigate('about')} className="rounded-full border-2 border-line bg-white px-6 py-3 font-bold text-ink-soft transition hover:border-npurple hover:text-npurple">
                About USAII&reg;
              </button>
            </motion.div>
                ...
                <AnimatePresence mode="wait">
                  {success ? (
                    ...
                    <motion.div key="form" exit={{ opacity: 0, y: -10 }}>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-1.5 rounded-2xl bg-mist p-1.5" role="tablist" aria-label="Choose how you are signing in">
                        {ROLES.map((r) => {
                          const on = role === r.id;
                          return (
                            <button
                              ...
                              className={cx('relative flex items-center justify-center gap-2 rounded-xl px-2 py-2.5 text-[13px] font-bold transition', on ? 'text-white' : 'text-ink-soft hover:text-ink')}
                            >
                      ...
                      {role === 'learner' && (
                        <p className="mt-4 text-center text-xs text-ink-faint">
                          New to USAII&reg;?{' '}
                          <button onClick={() => navigate('explore-courses')} className="font-bold text-npurple hover:text-nblue">
                            Explore Courses
                          </button>{' '}
                          and enroll in minutes.
                        </p>
                      )}
                    </motion.div>
                  )}
